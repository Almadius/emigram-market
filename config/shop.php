...
return [
	...
	'routes' => [
		...
		// Отключаем все маршруты Aimeos, так как используем свой SPA frontend
		'admin' => false,
		'jqadm' => false,
		'graphql' => false,
		'jsonadm' => false,
		'jsonapi' => false, // Можно оставить для API если нужно
		'account' => false,
		'default' => false,
		'basket' => false,
		'checkout' => false,
		...
		'supplier' => false,
		'page' => false,
		'home' => false, // Отключаем главный маршрут, чтобы не конфликтовал с SPA
		'update' => false,
	],
	...
	'resource' => [
		'db' => [
			'adapter' => config( 'database.connections.' . config( 'database.default', 'sqlite' ) . '.driver', 'sqlite' ),
			'host' => config( 'database.connections.' . config( 'database.default', 'sqlite' ) . '.host', '127.0.0.1' ),
			'port' => config( 'database.connections.' . config( 'database.default', 'sqlite' ) . '.port', '' ),
			'socket' => config( 'database.connections.' . config( 'database.default', 'sqlite' ) . '.unix_socket', '' ),
			'database' => config( 'database.connections.' . config( 'database.default', 'sqlite' ) . '.database', database_path('database.sqlite') ),
			'username' => config( 'database.connections.' . config( 'database.default', 'sqlite' ) . '.username', '' ),
			...
			'driverOptions' => config( 'database.connections.' . config( 'database.default', 'sqlite' ) . '.options', [] ),
		],
		'fs' => [
			'adapter' => 'Standard',
